...
public class NotificationReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Intent attachmentIntent = new Intent(context, NoticeDetailActivity.class);
        switch (MyMemory.getInstance().getVisible_status()) {
                //进入到activity
            case NoticeDetailActivity.OTHER:
                ...
                break;
                //直接关闭ntf就好了
            case NoticeDetailActivity.ATTACHMENT_VISIBLE:
                attachmentIntent = null;
                break;
            case NoticeDetailActivity.ATTACHMENT_INVISIBLE:
                //该属性由后台切入前台时使用
                attachmentIntent.addFlags(Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
                break;
                //切换至前台后切换item
            case NoticeDetailActivity.NOTICE_INVISIBLE:
                attachmentIntent.addFlags(Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
                MyMemory.getInstance().getUpdate().setType(Update.updateItem);
                break;
                //使用观察者，让它切换item
            case NoticeDetailActivity.NOTICE_VISIBLE:
                attachmentIntent=null;
                MyMemory.getInstance().getUpdate().setType(Update.updateItem);
                break;
        }
